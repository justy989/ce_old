...
bool ce_draw_buffer(const Buffer_t* buffer, Point_t cursor, Point_t term_top_left,
                    Point_t term_bottom_right, Point_t buffer_top_left, const char* highlight_word)
{
...
void ce_move_cursor_to_beginning_of_file(Point_t* cursor)
{
     *cursor = (Point_t) {0, 0};
}
...
void calc_vertical_views(BufferView_t* view, Point_t top_left, Point_t bottom_right, bool already_calculated);

void calc_horizontal_views(BufferView_t* view, Point_t top_left, Point_t bottom_right, bool already_calculated)
{
...
void ce_calc_views(BufferView_t* view, Point_t top_left, Point_t bottom_right)
{
...
void ce_connect_border_lines(Point_t location)
{
...
void connect_borders(const BufferView_t* view)
{
     ...
     ce_connect_border_lines(top_left);
     ce_connect_border_lines(top_right);
     ce_connect_border_lines(bottom_right);
     ce_connect_border_lines(bottom_left);
}
...
int64_t ce_compute_length(const Buffer_t* buffer, Point_t start, Point_t end)
{
     ...
     if(a->y < b->y){
          length = strlen(buffer->lines[a->y] + a->x) + 1; // account for newline
          for(int64_t i = a->y + 1; i < b->y; ++i){
               length += strlen(buffer->lines[i]) + 1; // account for newline
          }
          length += b->x+1; // do not account for newline. b->is inclusive
     }else{
          assert(a->y == b->y);
          length += b->x+1 - a->x;
     }
